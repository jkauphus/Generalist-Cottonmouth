...
```{r Cottonmouth Data, eval=FALSE, fig.height=8, fig.width=8, message=FALSE, warning=FALSE, cache=FALSE, include=FALSE}
extent <- as.double(extent(-94,-78,25,40))

# DISMO and GBIF
#Below, code of locations from the gbif database.

agk_dismo <- rgbif::occ_search(scientificName = "Agkistrodon piscivorus", geometry = extent)

gbif("agkistrodon", species = "piscivorus", ext = extent, geo = TRUE, sp = TRUE, download = TRUE, removeZeros = TRUE)
...
```{r Interactive Ploty Map, fig.width= 8, fig.height= 8, message=FALSE, warning=FALSE, cache=FALSE}
#Setting Up Coordinates
# Combine coordinates
coord <- cbind(study$UTM_mE, study$UTM_mN)

# Create sf object (UTM Zone 16N, WGS84)
coord_locations <- st_as_sf(
  data.frame(coord),
  coords = c("study.UTM_mE", "study.UTM_mN"),  # we’ll fix this below
  crs = 32616  # EPSG 32616 = WGS84 / UTM zone 16N
)
...
```{r Hydro Graphs, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
...
```
#### Colored tab
```{r colored hydrograph, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
hydrogrid
```
...
#### Black and White tab
```{r black and white hydrograph, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
hydrogrid1
```
...
```{r Mapping NLCD, message=FALSE, warning=FALSE}
...
#### Black and White tab
```{r PCA black and white, fig.height=10, fig.width=22, message=FALSE, warning=FALSE}
bw
```
...
```{r Variable importance Plot, fig.height=10, fig.width=15, message=FALSE, warning=FALSE}
Q
```
