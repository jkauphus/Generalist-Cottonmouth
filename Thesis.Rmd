...
```{r Video Code, message=FALSE, warning=FALSE}
video_path  <- "data/"
...
pp<-gg_scatter2+ geom_point(data = pcomp, aes(pcomp$Comp.1, pcomp$Comp.2, shape = Group, color =Group), alpha = 1, size =7)+
  scale_shape_manual(values = c(19,19,18,17))+
...
geom_density(data =group, aes(Comp.1, fill = group$Group, linetype = Group), alpha = 0.83, size =1)+
  scale_fill_manual(values = c("black", "#d9d9d9","#636363","#bdbdbd"))+
  scale_linetype_manual(values = c("blank", "solid", "dashed", "dotted"))

ydens1<-axis_canvas(pp, axis = "y", coord_flip = TRUE)+
geom_density(data =group, aes(Comp.2, fill = group$Group, linetype = Group), alpha = 0.83, size =1)+
  scale_fill_manual(values = c("black", "#d9d9d9","#636363","#bdbdbd"))+
  scale_linetype_manual(values = c("blank", "solid", "dashed", "dotted"))+
  coord_flip()+
  annotate("label", x = -2.4, y = 0.12, label = "FEMALE", color ="#252525", size = 3.2)+
  annotate("label", x = 2.5, y = 0.12, label = "JUVENILE", color ="#252525", size = 3.2, alpha = 0.25)+
...
#### Colored tab
```{r PCA colored, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
Colored
```
...
#### Black and White tab
```{r PCA black and white, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
bw
```
