...
hydrogrid<-plot_grid(p1,p2, ncol = 1, nrow = 2)
...
hydrogrid1<-plot_grid(p1,p2, ncol = 1, nrow = 2)

p_list[[1]] <-list(hydrogrid, "Colored Plot")
p_list[[2]] <-list(hydrogrid1, "Black and White Plot")
```
### Hydrology Plots {.tabset}
```{r tabs script, fig.height=8, fig.width=10}
...
The first three principal components describing cottonmouth habitat choice collectively explained 43.7% of the variation within the environmental variable structure (Table 1). Environmental variables associated with PC-1 describe a terrestrial to aquatic gradient (Figure 2) where distance to water was positively associated, while percent water and water depth were negatively associated. PC-2 described availability of cover objects, where log diameter and fallen log cover were positively associated and PC-3 described vegetation cover, where vegetation cover was positively associated with cottonmouth habitat. Collectively, the three components explain an environmental gradient between flooded and non-flooded habitats along with available refuge cover, where cottonmouths had broad associations between the extremes of aquatic and terrestrial sites, and slight preferences to cover objects.

The PERMANOVA/PERMDISP tests found a difference between cottonmouth habitat selection and available habitat within multivariate space (PERMANOVA; P< 0.001; Pseudo-F = 20.4; df = 1). Specifically, the results from the PERMDISP test showed that differences between cottonmouth sites and available habitat sites were attributed to the heterogeneous distribution of variance of the environmental variables between the two groups (PERMDISP; P<0.01; F = 25.44; df = 1). The average distance to centroid location among ellipsoids in multivariate space (Figure 2), based on PERMDISP, found that available habitat was 4.02, while cottonmouth habitat was 3.26. The PERMANOVA/PERMDISP tests confirmed observations from PCA, in that the heterogeneous dispersion between cottonmouth habitat selection and available habitat showed that cottonmouths used habitat resources differently than the occurrence of available habitat in the environment (Figure 2). In addition, the restricted distribution of sites used by cottonmouths, in relation to available habitat, was due to avoidance of upland habitats during the active season in favor of occupying aquatic-associated sites (likely foraging sites) within the more hydrologically-dynamic slough.
...
gg_scatter2<-ggplot(data = s1, aes(Comp.1, Comp.2, color = Type))+
  geom_point(size = 3.1)+
  scale_color_manual(values = c("black","#252525","#d9d9d9","#636363","#bdbdbd","grey44"))+
  theme_bw(base_size = 24)+
  theme(panel.grid = element_blank(), panel.border = element_rect(fill= "transparent"))+
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2)+
  ...
  stat_ellipse(type = "norm", aes(fill = Type), 
               show.legend = FALSE,size = 1, level = 0.95, linetype = 2)+
  scale_fill_manual(values=c("#252525", "grey44"))+
...
pp<-gg_scatter2+ geom_point(data = pcomp, aes(pcomp$Comp.1, pcomp$Comp.2, shape = Group, color =Group), alpha = 1, size =7)+
  scale_shape_manual(values = c(15,19,18,17))+
  annotate("label", x = 0.5, y = 0.5, label = "FEMALE", color ="#252525", size = 4.4)+
  annotate("label", x = -0.9, y = 1.1, label = "JUVENILE", color ="#252525", size = 4.4)+
  annotate("label", x = -0.5, y = -0.1, label = "MALE", color ="#252525", size = 4.4)+
  annotate("label", x = 0.15, y = 1.65, label = "NEONATE", color ="#252525",  size = 4.4)

xdens1<-axis_canvas(pp, axis = "x")+
geom_density(data =group, aes(Comp.1, fill = group$Group), alpha = 0.80, size =1, linetype= "dashed")+
  scale_fill_manual(values = c("black", "#d9d9d9","#636363","#bdbdbd"))

ydens1<-axis_canvas(pp, axis = "y", coord_flip = TRUE)+
geom_density(data =group, aes(Comp.2, fill = group$Group), alpha = 0.80, size =1, linetype= "dashed")+
  scale_fill_manual(values = c("black", "#d9d9d9","#636363","#bdbdbd"))+
  coord_flip()

pp1<-insert_xaxis_grob(pp, xdens1, grid::unit(0.2, "null"), position= "top")
pp2<-insert_yaxis_grob(pp1,ydens1, grid::unit(0.2, "null"), position = "right")

bw<-ggdraw(pp2)

#Merging into tab
pp_list[[1]] <-list(Colored, "Colored Plot")
pp_list[[2]] <-list(bw, "Black and White Plot")
```

### PCA Plots {.tabset}
```{r PCA tabs script, fig.height=8, fig.width=10}
...
**Figure 2:** Principal component analysis of the environmental variables measured for A. piscivorus (cottonmouth) and the available habitat comparison sites. Ellipses represent 95% confidence intervals for cottonmouth (grey) and available habitat (black). Mean principal component scores of the intra-specific cottonmouth conspecifics; adult males (blue), adult females (red), juveniles (green), and neonates (purple). Density counts of the cottonmouth conspecifics represented for each PC score, with PC-1 cottonmouth conspecific density above the plot, and PC-2 cottonmouth conspecific density toward the right of the plot.
