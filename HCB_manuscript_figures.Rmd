...
ggsave("HCB_figs/figS1_hydrographs.jpg")
...
#### Colored PCA Plot
```{r PCA colored, fig.height= 8, fig.width=22, message=FALSE, warning=FALSE}
...
ggsave("HCB_figs/fig2_PCA_ellipses.jpg")
```
...
ggsave("HCB_figs/fig3_SVL_PCA_regressions.jpg")
```
...
```{r Variable Importance, fig.width=7.2, message=FALSE, warning=FALSE}
...
ggplot(D, aes(x = reorder(Habitat_Variables, D$SNAKE), y = D$SNAKE))+
  ...
  geom_text(label=round(D$SNAKE,1), nudge_y = 0.5, size = 4, family = "Times New Roman")+
    #labs(subtitle = "Snake Model")+
...
ggsave("HCB_figs/fig4_variable_importance.jpg")
```
...
If you want readers to see the actual model-predicted partial probabilities, use: center = "FALSE" in setting up partial dependence plots.

a7 is the RF model with highest accuracy.

```{r Partial Dependence Plots Set up, message=FALSE, warning=FALSE}
#RF a7
plotx<-partial(a7, pred.var = "Dist..To.Water", pdp = T, center = T, plot = T, plot.engine = "ggplot2", type = "classification", which.class = "SNAKE", train = train, rug = F)

ploty<-partial(a7, pred.var = "X.Bare.soil", pdp = T, center = T, plot = T, plot.engine = "ggplot2", type = "classification", which.class = "SNAKE", train = train, rug = F)

plotz<-partial(a7, pred.var = "Rock.Cover", pdp = T, center = T, plot = T, plot.engine = "ggplot2", type = "classification", which.class = "SNAKE", train = train, rug = F)

plotz1<-partial(a7, pred.var = "Depth", pdp = T, center = T, plot = T, plot.engine = "ggplot2", type = "classification", which.class = "SNAKE", train = train, rug = F)

plotz2<-partial(a7, pred.var = "X.Water", pdp = T, center = T, plot = T, plot.engine = "ggplot2", type = "classification", which.class = "SNAKE", train = train, rug = F)

plotz3<-partial(a7, pred.var = "Dist..To.Cover", pdp = T, center = T, plot = T, plot.engine = "ggplot2", type = "classification", which.class = "SNAKE", train = train, rug = F)
```

```{r Partial Dependence Plots, message=FALSE, warning=FALSE}

theme_partial_dependence <- theme_bw() +
  theme(
    text = element_text(family = "Times New Roman"),
    axis.title.x = element_text(size = 12),
    axis.title.y = element_text(size = 12),
...
(pd_plot1 + pd_plot2 + pd_plot3) / (pd_plot4 + pd_plot5 + pd_plot6) +
  plot_annotation(tag_levels = 'A')

ggsave("HCB_figs/fig5_partial_dependence_plots.jpg")
